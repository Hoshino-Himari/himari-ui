...
// 移植自 GodUI <https://godui.design/docs/components/layout/image-accordion> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整（語意色 class 改為 var(--x, fallback)）。

import * as React from "react";

/**
 * 圖片手風琴：一排並列的圖片面板，滑到哪一片就展開哪一片，同時恢復飽和度並帶出標題與說明。
 * 手機上自動轉為直向排列。
 * @example
 * <ImageAccordion
 *   panels={[
 *     { image: "/a.jpg", title: "山", description: "清晨的稜線" },
 *     { image: "/b.jpg", title: "海" },
 *   ]}
 * />
 */
        ...
        {panels.map((panel, i) => {
          ...
          return (
            <Tag
              ...
              onClick={() => setActive(i)}
              onFocus={() => setActive(i)}
              className="group relative block min-h-[3rem] min-w-0 overflow-hidden rounded-2xl text-left [transition:flex-grow_550ms_cubic-bezier(0.22,1,0.36,1)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring,oklch(0.708_0_0))] motion-reduce:transition-none sm:min-h-0 sm:min-w-[2rem]"
