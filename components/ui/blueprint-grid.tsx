...
// 移植自 GodUI <https://godui.design/docs/components/backgrounds/blueprint-grid> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整：語意色改為 var(--x, fallback)，
// 掃光 keyframes 改為內嵌（GodUI 的 animate-blueprint-sweep 定義在未隨元件散佈的主題 CSS 裡）。

import * as React from "react";

/**
 * 藍圖格線：工程製圖風的格線背景，可選直線、點陣或會往地平線退去的透視地板，
 * 上頭有一道緩慢斜掃的光帶，游標經過的格子還會亮起來。
 * @example
...
export type BlueprintGridProps = React.HTMLAttributes<HTMLDivElement> & {
  ...
  /** Cell size in px. */
  cellSize?: number;
  /** Line / dot color, any CSS color string. */
  color?: string;
  ...
  spotlight?: boolean;
  /** Color the spotlit cells light up in. */
  ...
  (
    {
      ...
      sweepDuration = 8,
      spotlight = true,
      spotlightColor = "var(--primary, oklch(0.205 0 0))",
      spotlightRadius = 200,
      ...props
    },
    ref,
    ...
    return (
      ...
      >
        ...
        {sweep && (
          <div
            className="himari-blueprint-sweep absolute top-0 left-0 h-[140%] w-[55%] [will-change:transform] motion-reduce:hidden"
            style={
              {
        ...
        <style>{`
          .himari-blueprint-sweep {
            animation: himari-blueprint-sweep var(--blueprint-speed, 8s) linear infinite;
          }
          @keyframes himari-blueprint-sweep {
            0% { transform: translate3d(-120%, -20%, 0) rotate(16deg); }
            100% { transform: translate3d(320%, -20%, 0) rotate(16deg); }
          }
          @media (prefers-reduced-motion: reduce) {
            .himari-blueprint-sweep { animation: none; }
          }
        `}</style>
      </div>
    );
