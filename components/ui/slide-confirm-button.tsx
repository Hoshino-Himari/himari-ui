...
// 移植自 GodUI <https://godui.design/docs/components/buttons/slide-confirm-button> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整。
/**
 * 滑動確認按鈕：把圓形滑塊拖到軌道尾端才觸發 onConfirm，沒拖到門檻會彈回。
 * onConfirm 可以回傳 Promise，等待期間滑塊顯示轉圈，完成後打勾。
 * 適合付款、轉帳、刪除等不容誤觸的關鍵操作。
 * @example
 * <SlideConfirmButton label="滑動以確認" onConfirm={async () => await pay()} />
 */
...
const trackVariant: Record<SlideConfirmVariant, string> = {
  default:
    "border-[var(--border,oklch(0.922_0_0))] bg-[var(--muted,oklch(0.97_0_0))]",
  destructive:
    "border-[var(--destructive,oklch(0.577_0.245_27.325))]/30 bg-[var(--destructive,oklch(0.577_0.245_27.325))]/10",
};

const fillVariant: Record<SlideConfirmVariant, string> = {
  default: "bg-[var(--primary,oklch(0.205_0_0))]/20",
  destructive: "bg-[var(--destructive,oklch(0.577_0.245_27.325))]/25",
};
...
const thumbVariant: Record<SlideConfirmVariant, string> = {
  ...
  destructive: "bg-[var(--destructive,oklch(0.577_0.245_27.325))] text-white",
};
    ...
    return (
      ...
      >
        ...
        <motion.span
          ...
          className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center whitespace-nowrap font-medium text-[var(--muted-foreground,oklch(0.556_0_0))]"
        >
          {label}
        </motion.span>
        {busy && (
          <span
            ...
            className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center whitespace-nowrap font-medium text-[var(--foreground,oklch(0.145_0_0))]"
          >
        ...
        <motion.button
          ...
          className={`relative z-20 inline-flex shrink-0 cursor-grab items-center justify-center rounded-full shadow-sm active:cursor-grabbing focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring,oklch(0.708_0_0))] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--background,oklch(1_0_0))] ${thumbVariant[variant]}`}
        >
          {loading ? (
            <svg
              viewBox="0 0 24 24"
