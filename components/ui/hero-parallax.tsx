...
// 移植自 GodUI <https://godui.design/docs/components/layout/hero-parallax> — MIT License © LucasBassetti
...
/**
 * 英雄視差：捲動時三排卡片牆在透視平面上左右漂移、整體翻正淡入的首屏展示區。
 * @example
 * <HeroParallax products={items} scrollContainer={scrollRef} />
 */
  ...
  return (
    ...
    >
      <img
        ...
        alt={product.title}
        draggable={false}
        className="absolute inset-0 size-full rounded-xl object-cover shadow-lg [outline:1px_solid_color-mix(in_oklch,var(--border,#3f3f46)_60%,transparent)] [outline-offset:-1px]"
      />
  ...
  ({ products, header, className, scrollContainer, ...props }, ref) => {
    ...
    const defaultHeader = (
      <div className="relative top-0 left-0 mx-auto w-full max-w-7xl px-4 py-20 md:py-40">
        <h2 className="text-2xl font-bold text-[var(--foreground,#fafafa)] md:text-7xl">
          The ultimate <br /> showcase reel
        </h2>
        <p className="mt-8 max-w-2xl text-base text-[var(--muted-foreground,#a1a1aa)] md:text-xl">
