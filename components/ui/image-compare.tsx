...
// 移植自 GodUI <https://godui.design/docs/components/layout/image-compare> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整。

import * as React from "react";

/**
 * 影像比較滑桿：拖曳分隔把手（或用方向鍵）在 before / after 兩層內容之間滑動對比。
 * @example
 * <ImageCompare before={<img src="/before.jpg" alt="" />} after={<img src="/after.jpg" alt="" />} />
 */
  ...
  (
    ...
    return (
      <div
        ...
        className={`group relative touch-none select-none overflow-hidden rounded-xl border border-[var(--border,#3f3f46)] ${
          className ?? ""
        }`}
        {...props}
      >
        ...
        <button
          ...
          style={handleStyle}
          className="absolute grid size-9 cursor-grab touch-none place-items-center rounded-full border-2 border-white bg-white/20 text-white shadow-lg backdrop-blur-sm [transition:scale_150ms_ease] active:scale-95 active:cursor-grabbing focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ring,#71717a)] focus-visible:ring-offset-2 group-hover:scale-110"
        >
          <span className="absolute inset-0 animate-pulse rounded-full ring-2 ring-white/40 group-hover:hidden motion-reduce:animate-none" />
          <svg
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
