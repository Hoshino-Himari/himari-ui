...
// 移植自 GodUI <https://godui.design/docs/components/effects/fluid-cursor> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整（z-toast 改為 z-50）。

import * as React from "react";
import { createPortal } from "react-dom";

/**
 * 流體游標：一顆貼著游標跑的圓點，後面拖著一團慢半拍的模糊光暈，
 * 滑到連結或按鈕上會放大；預設用 difference 混合模式把底下的畫面反相。
 * 只在精準指標裝置上啟用，並尊重減少動態設定。
 * @example
 * <div className="relative h-96">
 *   <FluidCursor containerRef={boxRef} />
 * </div>
 */
...
const FluidCursor = ({
  ...
  return createPortal(
    <div
      ...
      className={`pointer-events-none ${scoped ? "absolute" : "fixed"} inset-0 z-50 overflow-hidden ${blend ? "mix-blend-difference" : ""} ${className ?? ""}`}
    >
      {trail ? (
        <div
          ref={trailRef}
