...
// 移植自 GodUI <https://godui.design/docs/components/backgrounds/pixel-grid> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整：色彩探針改用 var(--foreground, fallback) 取代 text-foreground class。

import * as React from "react";

/**
 * 像素格網：整片小方塊隨機閃爍的 canvas 背景，互動模式下只有游標半徑內的方塊會亮起來，
 * 像一盞跟著滑鼠走的探照燈。放在 `relative` 容器的第一個子元素。
 * @example
 * <div className="relative h-96 overflow-hidden">
 *   <PixelGrid color="#f59e0b" />
 * </div>
 */
export type PixelGridProps = React.HTMLAttributes<HTMLDivElement> & {
   ...
   * Defaults to the `--foreground` token, re-resolved on theme change so it
...
const PixelGrid = React.forwardRef<HTMLDivElement, PixelGridProps>(
  (
    ...
    // `--foreground` token and re-resolve whenever the theme flips.
    React.useEffect(() => {
      ...
      const resolve = () => {
        ...
        probe.style.cssText =
          "position:absolute;width:0;height:0;opacity:0;color:var(--foreground, oklch(0.145 0 0))";
