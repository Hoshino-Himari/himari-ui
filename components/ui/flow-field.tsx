...
// 移植自 GodUI <https://godui.design/docs/components/backgrounds/flow-field> — MIT License © LucasBassetti
// 保留原始實作，僅做自足化最小調整（語意色探針補上 fallback 值）。

import * as React from "react";

/**
 * 流場：上千顆粒子順著會演化的雜訊向量場流動，拖出綢緞般的漸隱軌跡，
 * 背景色也自動跟著主題走。放在 `relative` 容器的第一個子元素。
 * @example
 * <div className="relative h-96 overflow-hidden">
 *   <FlowField color="#f59e0b" />
 * </div>
 */
export type FlowFieldProps = React.HTMLAttributes<HTMLDivElement> & {
  ...
  speed?: number;
  /**
   * Trail color, any CSS color string. Defaults to the `--primary`
      ...
      const resolve = () => {
        fgRef.current = color
          ...
          : (() => {
              ...
              p.style.cssText =
                "position:absolute;width:0;height:0;opacity:0;color:var(--primary, oklch(0.205 0 0))";
        ...
        pb.style.cssText =
          "position:absolute;width:0;height:0;opacity:0;background:var(--background, oklch(1 0 0))";
